...
#        print "no clear gap, using default vel: {0:6.3f}  {1:6.3f}".format(desired_x,desired_y)
        ...
        if msg_ranges[4] < range_limit or range_history[i] < range_limit or range_history[9+4] < range_limit or range_history[18+4] < range_limit or range_history[27+4] < range_limit  :
            ...
            for i in range(2, 4):
                if msg_ranges[i] < range_limit or range_history[i] < range_limit or range_history[i+9] < range_limit or range_history[i+18] < range_limit or range_history[i+27] < range_limit:
                    desired_y = 0.1
                    desired_x = 0
    elif len(range_history) > 18 :
        if msg_ranges[4] < range_limit or range_history[i] < range_limit or range_history[9+4] < range_limit or range_history[18+4] < range_limit:
            desired_x = 0
        else:
            for i in range(0, 2):
                if msg_ranges[i] < range_limit or range_history[i] < range_limit or range_history[i+9] < range_limit or range_history[i+18] < range_limit:
                    desired_y = 0
                    print "low scans too close"
            for i in range(7, 9):
                if msg_ranges[7] < range_limit or range_history[7] < range_limit or range_history[7+9] < range_limit or range_history[7+18] < range_limit:
                    desired_y = 0
                    print "high scans too close"
            for i in range(5, 7):
                if msg_ranges[i] < range_limit or range_history[i] < range_limit or range_history[i+9] < range_limit or range_history[i+18] < range_limit:
            ...
            for i in range(2, 4):
                if msg_ranges[i] < range_limit or range_history[i] < range_limit or range_history[i+9] < range_limit or range_history[i+18] < range_limit:
                    desired_y = 0.1
                    desired_x = 0
    elif len(range_history) > 9 :
        if msg_ranges[4] < range_limit or range_history[i] < range_limit or range_history[9+4] < range_limit:
            desired_x = 0
        else:
            for i in range(0, 2):
                if msg_ranges[i] < range_limit or range_history[i] < range_limit or range_history[i+9] < range_limit:
                    desired_y = 0
                    print "low scans too close"
            for i in range(7, 9):
                if msg_ranges[7] < range_limit or range_history[7] < range_limit or range_history[7+9] < range_limit:
                    desired_y = 0
                    print "high scans too close"
            for i in range(5, 7):
                if msg_ranges[i] < range_limit or range_history[i] < range_limit or range_history[i+9] < range_limit:
    ...
    if len(range_history) > (5*9):
        for i in range(len(range_history), 5*9):
            del range_history[i]
